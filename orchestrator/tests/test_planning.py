...
class TestReasoning:
    ...
    async def test_reason_success(
        ...
        with patch("src.orchestrator.planning.llm_groq", mock_llm):
            result = await reason(sample_chatbot_query)
        ...
        with patch("src.orchestrator.planning.llm_groq", mock_llm):
            with patch("src.orchestrator.planning.REASONING_ATTEMPTS", 3):
                with pytest.raises(ReasoningException) as excinfo:
                    await reason(sample_chatbot_query)
...
class TestAct:
    ...
    async def test_act_easy(
        self, mock_answer, sample_chatbot_query, sample_reasoning_outcome_easy
    ):
        mock_answer.return_value = None
        await act(sample_reasoning_outcome_easy, sample_chatbot_query)
        mock_answer.assert_called_once_with(sample_chatbot_query)
    ...
    async def test_act_medium(
        ...
        await act(sample_reasoning_outcome_medium, sample_chatbot_query)
        mock_ask_single_doctor.assert_called_once_with(
            sample_chatbot_query, sample_reasoning_outcome_medium.diseases[0].disease
        )

    @pytest.mark.asyncio
    @patch("src.orchestrator.planning.ask_many_doctors")
    async def test_act_hard(
        self, mock_ask_many_doctors, sample_chatbot_query, sample_reasoning_outcome_hard
    ):
        mock_ask_many_doctors.return_value = None
        await act(sample_reasoning_outcome_hard, sample_chatbot_query)
        mock_ask_many_doctors.assert_called_once_with(
            sample_chatbot_query, sample_reasoning_outcome_hard.diseases
        )

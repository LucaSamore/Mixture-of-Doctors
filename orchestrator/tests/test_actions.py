...
class TestRagModuleInteractions:
    ...
    async def test_ask_single_doctor(self, sample_chatbot_query, mock_kafka_producer):
        ...
        disease = "diabetes"
        with patch("src.orchestrator.planning.uuid.uuid4", return_value="test-uuid"):
            await ask_single_doctor(sample_chatbot_query, disease, mock_kafka_producer)
    ...
    @pytest.mark.asyncio
    async def test_ask_many_doctors(self, sample_chatbot_query, mock_kafka_producer):
        ...
        with patch("src.orchestrator.planning.uuid.uuid4", return_value="test-uuid"):
            await ask_many_doctors(
                sample_chatbot_query, disease_questions, mock_kafka_producer
            )
            assert mock_kafka_producer.send_and_wait.call_count == 2
            # Check first call (diabetes)
            first_call_args = mock_kafka_producer.send_and_wait.call_args_list[0]
            ...
            assert (
                message1["total"] == 2
            )  # Should be the actual number of diseases in the question
            # Check second call (hypertension)
            second_call_args = mock_kafka_producer.send_and_wait.call_args_list[1]

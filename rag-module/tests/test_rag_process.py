...
class TestRagProcess:
    @patch("rag_module.rag_process.SentenceTransformer")
    def test_retrieve(self, mock_transformer, mock_qdrant_client):
        ...
        with patch("rag_module.rag_process.qdrant_client", mock_qdrant_client):
    ...
    async def test_augment(self, sample_conversation_items):
        ...
        with (
            patch(
                "rag_module.rag_process.fetch_chat_history_for_user", mock_fetch_history
            ),
    ...
    @patch("rag_module.rag_process.llm_groq_client")
    @patch("rag_module.rag_process.redis_client")
    ...
    @patch("rag_module.rag_process.llm_groq_client")
    @patch("rag_module.rag_process.kafka_client")
